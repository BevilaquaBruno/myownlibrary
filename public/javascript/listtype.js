...
function getDataForTableType() {
  ...
  data.then(dt => {
    ...
    dt.forEach(el => {
      ...
      html += '<a title="Excluir Tipo" href="/tipo/excluir/'+el._id+'" class="btn btn-red btn-small"><img src="/public/imgs/trash.svg"></a></td>';
      html += '<td style="display: none;">'+el._id+'</td></tr>';
    });
    document.getElementById('tableTypeBody').innerHTML = html;
  });
  TableSorter.makeSortable(document.getElementById('tableType'));
}
